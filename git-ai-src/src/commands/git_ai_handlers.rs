...
use crate::authorship::stats::{stats_command, stats_for_commit_stats};
...
use crate::git::repository::{CommitRange, Repository, exec_git, group_files_by_repository};
...
fn print_help() {
    ...
    eprintln!("    --all                  Show AI code adoption rate across entire repo history");
    eprintln!("    --json                 Output in JSON format");
...
fn handle_stats(args: &[String]) {
    ...
    // --tracked-only: only count commits that have git-ai authorship notes.
    // Uses per-commit stats aggregation so diff numbers are scoped correctly.
    if tracked_only && (all_commits || commit_range.is_some()) {
        let shas = if all_commits {
            rev_list_filtered(&repo, "4b825dc642cb6eb9a060e54bf8d69288fbee4904", &resolve_head_sha(&repo), no_merges)
        } else {
            let range = commit_range.as_ref().unwrap();
            rev_list_filtered(&repo, &range.start_oid, &range.end_oid, no_merges)
        };

        // Filter to only commits with authorship notes
        let tracked_shas: Vec<String> = shas
            .into_iter()
            .filter(|sha| crate::git::refs::get_authorship(&repo, sha).is_some())
            .collect();

        if tracked_shas.is_empty() {
            if json_output {
                println!("{}", serde_json::to_string(&crate::authorship::stats::CommitStats::default()).unwrap());
            } else {
                eprintln!("No tracked commits found (no authorship notes).");
            }
            return;
        }

        // Aggregate per-commit stats
        let mut total = crate::authorship::stats::CommitStats::default();
        for sha in &tracked_shas {
            match stats_for_commit_stats(&repo, sha, &effective_patterns) {
                Ok(s) => {
                    total.human_additions += s.human_additions;
                    total.mixed_additions += s.mixed_additions;
                    total.ai_additions += s.ai_additions;
                    total.ai_accepted += s.ai_accepted;
                    total.total_ai_additions += s.total_ai_additions;
                    total.total_ai_deletions += s.total_ai_deletions;
                    total.time_waiting_for_ai += s.time_waiting_for_ai;
                    total.git_diff_added_lines += s.git_diff_added_lines;
                    total.git_diff_deleted_lines += s.git_diff_deleted_lines;
                    for (key, tool_stats) in s.tool_model_breakdown {
                        let entry = total.tool_model_breakdown.entry(key).or_default();
                ...
                Err(e) => {
                    eprintln!("Warning: failed to get stats for {}: {}", &sha[..7], e);
                }
            }
        }

        if json_output {
            println!("{}", serde_json::to_string(&total).unwrap());
        } else {
            crate::authorship::stats::write_stats_to_terminal(&total, true);
            println!("  {} tracked commit(s)", tracked_shas.len());
        }
        return;
    }

    // Handle --all: aggregate stats across entire repo history
    if all_commits {
        let empty_tree = "4b825dc642cb6eb9a060e54bf8d69288fbee4904".to_string();
        let head_sha = resolve_head_sha(&repo);
        let head = repo.head().unwrap();
        let refname = head.name().unwrap_or("HEAD").to_string();

        let range = match CommitRange::new(&repo, empty_tree.clone(), head_sha.clone(), refname) {
            Ok(r) => r,
            Err(e) => {
                eprintln!("Failed to create commit range: {}", e);
                std::process::exit(1);
        ...
        let commit_shas = if no_merges {
            Some(rev_list_filtered(&repo, &empty_tree, &head_sha, true))
        } else {
            None
        };

        match range_authorship::range_authorship(range, false, &effective_patterns, commit_shas) {
            Ok(stats) => {
                if json_output {
                    let json_str = serde_json::to_string(&stats).unwrap();
                    println!("{}", json_str);
                } else {
                    crate::authorship::stats::print_adoption_rate(&stats.range_stats);
    ...
    if let Some(range) = commit_range {
        let commit_shas = if no_merges {
            Some(rev_list_filtered(&repo, &range.start_oid, &range.end_oid, true))
        } else {
            None
        };

        match range_authorship::range_authorship(
            range,
            false,
            &effective_patterns,
            commit_shas,
...
/// Run `git rev-list` with optional `--no-merges` and return the list of commit SHAs.
fn rev_list_filtered(repo: &Repository, start: &str, end: &str, no_merges: bool) -> Vec<String> {
    ...
    match exec_git(&args) {
        Ok(output) => String::from_utf8(output.stdout)
            ...
            .collect(),
        Err(e) => {
            eprintln!("Failed to run rev-list: {}", e);
            Vec::new()
        }
    }
}
...
fn handle_post_commit(args: &[String]) {
    ...
    let mut repo = match find_repository(&[]) {
        Ok(repo) => repo,
        Err(e) => {
            eprintln!("Failed to find repository: {}", e);
            std::process::exit(1);
    ...
    // Get parent commit SHA via `git rev-parse <commit>^`
    // For initial commits (no parent), set to None
    let base_commit: Option<String> = {
        let mut args = repo.global_args_for_exec();
        args.push("rev-parse".to_string());
        args.push("--verify".to_string());
        args.push(format!("{}^", commit_sha));
        match exec_git(&args) {
            Ok(output) => {
                let parent = String::from_utf8_lossy(&output.stdout).trim().to_string();
                if parent.is_empty() { None } else { Some(parent) }
            }
            Err(_) => None, // Initial commit has no parent
        }
    };

    // Get author identity formatted as "name <email>"
    let identity = repo.git_commit_author_identity();
    let author = match (identity.name, identity.email) {
    ...
    // Run pre-commit checkpoint to capture any human edits made after the last
    // AI checkpoint. In the old git.path proxy mode this was handled by the
    // pre-commit hook before `git commit` ran. Now that we no longer intercept
    ...
    if let Err(e) = crate::commands::checkpoint::run_with_base_commit_override(
        ...
        crate::utils::debug_log(&format!(
            "[post-commit] pre_commit checkpoint failed (non-fatal): {}",
            e
        ));
    }
