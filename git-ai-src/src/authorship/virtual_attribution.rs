            ...
            for entry in &checkpoint.entries {
                ...
                if let Ok(workdir) = repo.workdir() {
                    ...
                    let file_content = if abs_path.exists() {
                        ...
                        // File doesn't exist at original path — it may have been renamed/moved.
                        // Fall back to the blob content stored in the working log.
                        working_log
                ...
                if line_attrs.is_empty() {
                    ...
                    //
                    // EXCEPTION: If the file content is empty (file doesn't exist on disk,
                    // e.g., it was renamed/moved), don't clear attributions — the empty
                    // content is not a reliable signal that AI lines were removed.
                    // The file may still exist under a different path.
    ...
    pub fn from_working_log_snapshot(
        ...
        for checkpoint in &checkpoints {
            ...
            for entry in &checkpoint.entries {
                ...
                if line_attrs.is_empty() {
                    // Don't clear attributions when file content is empty (file may
                    // have been renamed/moved — empty content is not reliable signal).
                    if file_content.is_empty() {
                        continue;
                    }
                    attributions.remove(&entry.file);
...
impl VirtualAttributions {
    ...
    > {
        ...
        // Persistent debug
        if let Ok(workdir) = repo.workdir() {
            let debug_path = workdir.join(".git").join("ai").join("post_commit_debug.log");
            let _ = std::fs::OpenOptions::new().append(true).create(true).open(&debug_path).and_then(|mut f| {
                use std::io::Write;
                let attr_keys: Vec<&String> = self.attributions.keys().collect();
                let hunk_keys: Vec<&String> = committed_hunks.keys().collect();
                writeln!(f, "to_authorship_log:\n  attr_keys={:?}\n  committed_hunks_keys={:?}\n  pathspecs={:?}\n",
                    attr_keys, hunk_keys, pathspecs)
            });
        }

        // Detect file renames between parent and commit. When a file is renamed/moved,
        // committed_hunks uses the NEW path (from --no-renames: old deleted + new added),
        // but self.attributions uses the OLD path (from checkpoint before rename).
        // Build a mapping old_path -> new_path.
        //
        // Strategy: For each file in attributions that has no matching committed_hunks entry,
        // check if there's a committed_hunks entry for a file with the same basename.
        // This handles both simple renames (a.txt -> b.txt) and moves (dir1/a.txt -> dir2/a.txt).
        let rename_map: HashMap<String, String> = {
            let mut map = HashMap::new();
            // First try git's rename detection with a very low threshold
            if parent_sha != "initial"
                && let Ok(git_renames) = repo.diff_rename_map(parent_sha, commit_sha)
            {
                map.extend(git_renames);
            }
            // Fallback: for attribution files not in committed_hunks and not in git_renames,
            // try to match by filename against committed_hunks entries.
            // If basename doesn't match, use content-based matching: pair unmatched
            // attribution files with unmatched committed_hunks files.
            let mut unmatched_attrs: Vec<String> = Vec::new();
            for attr_path in self.attributions.keys() {
                if committed_hunks.contains_key(attr_path) || map.contains_key(attr_path) {
                    continue;
                }
                let attr_basename = attr_path.rsplit('/').next().unwrap_or(attr_path);
                let mut found = false;
                for committed_path in committed_hunks.keys() {
                    let committed_basename = committed_path.rsplit('/').next().unwrap_or(committed_path);
                    if attr_basename == committed_basename && !map.values().any(|v| v == committed_path) {
                        map.insert(attr_path.clone(), committed_path.clone());
                        found = true;
                        break;
                    }
                }
                if !found {
                    unmatched_attrs.push(attr_path.clone());
                }
            }

            // Last resort: for remaining unmatched attribution files, pair them with
            // unmatched committed_hunks files. This handles cases where both the filename
            // and directory changed (e.g., AI creates PrintFoo.java, user renames to Bar.java).
            //
            // Only consider attr files that have NO matching committed_hunks entry AND
            // don't exist in the commit tree. Files from INITIAL attributions that still
            // exist in the commit should NOT be paired.
            if !unmatched_attrs.is_empty() {
                let mut matched_committed: HashSet<String> = map.values().cloned().collect();
                let all_unmatched_committed: Vec<String> = committed_hunks.keys()
                    .filter(|p| !self.attributions.contains_key(*p) && !matched_committed.contains(*p))
                    .cloned()
                    .collect();

                // Filter: only attr files that don't have ANY committed hunks (not even
                // under a different case) are candidates for rename pairing.
                // Use git ls-tree to get all files in the commit, then do case-insensitive check.
                let commit_files: HashSet<String> = {
                ...
                let truly_missing_attrs: Vec<&String> = unmatched_attrs.iter()
                    .filter(|attr_path| {
                        // File is truly missing if it doesn't exist in commit tree
                        // (case-insensitive check for macOS compatibility)
                        !commit_files.contains(&attr_path.to_lowercase())
                    })
                    .collect();

                for attr_path in &truly_missing_attrs {
                    let available: Vec<&String> = all_unmatched_committed.iter()
                        .filter(|p| !matched_committed.contains(*p))
                        .collect();
                    if available.len() == 1 {
                        map.insert((*attr_path).clone(), available[0].clone());
                        matched_committed.insert(available[0].clone());
                    }
                }
            }
        ...
        for (file_path, (_, line_attrs)) in &self.attributions {
            ...
            // Resolve the effective path for this file in the commit.
            // If the file was renamed, use the new path to look up committed hunks.
            let effective_commit_path = rename_map
                .get(file_path)
                .cloned()
            ...
            if let Some(unstaged_ranges) = unstaged_hunks.get(file_path)
                .or_else(|| unstaged_hunks.get(&effective_commit_path))
            {
            ...
            // Try both the original path and the renamed path
            let file_committed_hunks = committed_hunks.get(file_path)
                .or_else(|| committed_hunks.get(&effective_commit_path));
            ...
            if !committed_lines_map.is_empty() {
                ...
                for (author_id, mut lines) in committed_lines_map {
                    ...
                    // Use the effective commit path (new path after rename) for the attestation
                    // so it matches the file path in git diff output for stats calculation.
                    let file_attestation = authorship_log.get_or_create_file(&effective_commit_path);
                    file_attestation.add_entry(entry);
                }
            }
